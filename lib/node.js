...
Object.defineProperties(SiteNode.prototype, assign({
	...
	_reset: d(function () {
		this.domMapReverse.forEach(function (conf) {
			var index;
			if (conf.content) {
				this.tree._resetElement(conf.element);
			} else {
				if (conf.prependMark) {
					index = 0;
					while (conf.element.childNodes[index] !== conf.prependMark) {
						this.tree._resetElement(conf.element.childNodes[index]);
						++index;
					}
				}
				if (conf.appendMark) {
					index = conf.element.childNodes.length - 1;
					while (conf.element.childNodes[index] !== conf.appendMark) {
						this.tree._resetElement(conf.element.childNodes[index]);
						--index;
					}
				}
