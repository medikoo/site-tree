	...
	map: d(function () {
		...
		forEach(this.conf, function (setup, id) {
			...
			if (typeof setup === 'function') {
				conf.content = this.tree.constructor.resolveTemplate(setup, this.context);
				return;
			}
			if (setup.class) conf.class = setup.class;
			if (setup.content) {
				conf.content = this.tree.constructor.resolveTemplate(setup.content, this.context);
			} else {
				if (setup.prepend) {
					conf.prepend = this.tree.constructor.resolveTemplate(setup.prepend, this.context);
				}
				if (setup.append) {
					conf.append = this.tree.constructor.resolveTemplate(setup.append, this.context);
				}
			}
		}, this);
		return map;
